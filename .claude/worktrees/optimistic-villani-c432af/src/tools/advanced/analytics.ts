  ...
  inputSchema: {
    ...
    properties: {
      siteId: {
        type: "string",
        description:
          "SharePoint site ID (optional, defaults to personal drive)",
      },
      site: {
        type: "string",
  ...
  try {
    ...
    const analyzeItems = async (
      ...
      for (const item of items) {
        if (item.file) {
          ...
          // File type analysis
          const ext =
            item.name.split(".").pop()?.toLowerCase() || "no_extension";
          ...
          if (lastModified < oldThreshold) {
            analytics.oldFiles.push({
              ...
              daysSinceModified: Math.floor(
                (now - lastModified) / (24 * 60 * 60 * 1000),
              ),
              webUrl: item.webUrl,
            });
          }
          ...
          if (includeVersions) {
            try {
              const versionsEndpoint = `${driveRoot}/items/${item.id}/versions`;
              ...
              if (versionsResponse.success && versionsResponse.data) {
                ...
                if (versions.length > 1) {
                  const versionSizes = versions.reduce(
                  ...
                  analytics.versionedFiles.push({
                    ...
                    oldestVersion:
                      versions[versions.length - 1]?.lastModifiedDateTime,
                  });
                }
              }
            } catch (versionError) {
          ...
          if (analysisType === "detailed" && depth < 3) {
            const subfolderEndpoint = `${driveRoot}/items/${item.id}/children`;

            const subAnalytics = await analyzeItems(
              subfolderEndpoint,
              depth + 1,
            );
    ...
    switch (analysisType) {
      ...
      case "detailed":
        results.detailed = {
          ...analytics,
          duplicates: Array.from<[string, any[]]>(
            analytics.duplicates.entries(),
          )
      ...
      case "duplicates":
        const duplicates = Array.from<[string, any[]]>(
          analytics.duplicates.entries(),
        )
          .filter(([_, files]: [string, any[]]) => files.length > 1)
          .map(([key, files]: [string, any[]]) => ({
        ...
        results.duplicates = {
          ...
          totalWastedSpace: duplicates.reduce(
            (sum: number, d: any) => sum + d.totalWastedSpace,
            0,
          ),
      ...
      case "large_files":
        results.largeFiles = {
          ...
          totalSize: analytics.largeFiles.reduce(
      ...
      case "old_files":
        results.oldFiles = {
          ...
          totalSize: analytics.oldFiles.reduce(
    ...
    if (includeVersions && analytics.versionedFiles.length > 0) {
      results.versions = {
        ...
        totalVersionSpace: analytics.versionedFiles.reduce(
          (sum: number, f: any) => sum + f.totalVersionSize,
          0,
        ),
...
export async function handleVersionManagement(args: any) {
  try {
    ...
    const baseEndpoint = `${driveRoot}/items/${actualItemId}`;
    ...
    switch (action) {
      case "list": {
        ...
        if (response.success && response.data) {
          ...
          return jsonTextResponse({
            ...
            versionCount: versions.length,
            totalVersionSize: versions.reduce(
              (sum: number, v: any) => sum + (v.size || 0),
              0,
            ),
      ...
      case "cleanup": {
        ...
        const spaceSaved = deletedVersions.reduce(
          (sum, v) => sum + (v.size || 0),
          0,
        );
      ...
      case "compare": {
        if (!versionId || !compareVersionId) {
          throw new Error(
            "versionId and compareVersionId are required for compare action",
          );
        }
...
export const analyticsTools = [storageAnalytics, versionManagement];

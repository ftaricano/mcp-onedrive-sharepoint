...
import { SecurityValidator, SecurePath, AuditLogger } from './security-validator.js';
import { CacheManager, metadataCache, searchCache, driveCache } from './cache-manager.js';
...
export class EnhancementTester {
  ...
  private async testSecurityValidation(): Promise<void> {
    ...
    await this.runTest('Path Traversal Detection', () => {
      ...
      if (result.isValid) throw new Error('Should have detected path traversal');
    });

    await this.runTest('Valid Path Acceptance', () => {
      const result = SecurityValidator.validatePath('documents/folder/file.txt');
    ...
    await this.runTest('Forbidden Extension Detection', () => {
      ...
      if (result.isValid) throw new Error('Should have rejected executable file');
    });
    ...
    await this.runTest('Script Injection Detection', () => {
      const result = SecurityValidator.validateSearchQuery('<script>alert("xss")</script>');
      if (result.isValid) throw new Error('Should have detected script injection');
    });
    ...
    await this.runTest('OData Injection Detection', () => {
      ...
      if (result.isValid) throw new Error('Should have detected OData injection');
    });
    ...
    await this.runTest('Valid OData Query', () => {
      ...
      if (!result.isValid) throw new Error('Should have accepted valid OData query');
    });
  }
  ...
  private async testCacheManager(): Promise<void> {
    ...
    await this.runTest('Cache LRU Eviction', () => {
      ...
      if (testCache.get('key1') !== null) throw new Error('LRU eviction failed');
      if (testCache.get('key4') !== 'value4') throw new Error('New key not stored');
    });
  ...
  private async testPathSecurity(): Promise<void> {
    ...
    await this.runTest('Path Traversal in Join', () => {
      ...
      if (result.isValid) throw new Error('Should have detected traversal in join');
    });
  ...
  private async testAuditLogging(): Promise<void> {
    ...
    await this.runTest('Basic Audit Log', () => {
      ...
      AuditLogger.log('test_operation', 'test_user', 'test_resource', 'success');
  ...
  async testCachePerformance(): Promise<void> {
    ...
    await this.runTest('Search Cache Integration', () => {
      ...
      const results = [{ id: '1', name: 'result1' }, { id: '2', name: 'result2' }];
   ...
   */
  private async runTest(testName: string, testFn: () => void | Promise<void>): Promise<void> {
    const start = Date.now();
    try {
      await testFn();
  ...
  static async quickValidation(): Promise<boolean> {
    try {
      ...
      const searchTest = SecurityValidator.validateSearchQuery('<script>alert(1)</script>');

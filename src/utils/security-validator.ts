...
export class SecurityValidator {
  ...
  static validateSearchQuery(query: string): ValidationResult {
    ...
    const sanitized = query.replace(/\s+/g, " ").trim().substring(0, 500);
...
export class AuditLogger {
  ...
  static log(
    ...
    const entry = {
      ...
      result,
      details: details
        ? SecurityValidator.sanitizeForLogging(details)
        : undefined,
    };
    ...
    console.log(
      `[AUDIT] ${entry.timestamp} | ${entry.operation} | ${entry.user} | ${entry.resource} | ${entry.result}`,
    );
  }

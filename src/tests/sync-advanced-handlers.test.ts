...
test('sync_folder covers upload conflict resolution branches', async () => {
  ...
  for (const scenario of scenarios) {
    ...
    try {
      ...
      writeFileWithMtime(localFilePath, 'local-content', scenario.localModified);
      ...
      assert.equal(payload.summary.uploaded, scenario.expectUpload ? 1 : 0, scenario.name);
      assert.equal(payload.summary.conflicts, scenario.expectConflict ? 1 : 0, scenario.name);
      assert.equal(mock.methodCalls('uploadFile').length, scenario.expectUpload ? 1 : 0, scenario.name);

      if (scenario.expectUpload) {
        const [uploadEndpoint, uploadPath, uploadName, options] = mock.methodCalls('uploadFile')[0].args;
        assert.equal(uploadEndpoint, '/me/drive/root:/Docs/report.txt:/content');
      ...
      if (scenario.expectConflict) {
        assert.match(payload.details.conflicts[0].newName, /^report_local_\d+\.txt$/);
      }
    } finally {
  ...
  for (const scenario of scenarios) {
    ...
    try {
      ...
      const mock = createMockGraphClient({
        ...
        downloadFile: async () => ({ success: true, data: Buffer.from('remote-buffer') }),
      });
      __setGraphClientInstanceForTests(mock.client as any);
      ...
      assert.equal(response.isError, undefined, scenario.name);
      assert.equal(payload.summary.downloaded, scenario.expectDownload ? 1 : 0, scenario.name);
      assert.equal(payload.summary.conflicts, scenario.expectConflict ? 1 : 0, scenario.name);
      assert.equal(mock.methodCalls('downloadFile').length, scenario.expectDownload ? 1 : 0, scenario.name);
      ...
      if (scenario.expectConflict) {
        assert.match(payload.details.conflicts[0].newName, /^report_remote_\d+\.txt$/);
      }
    } finally {
      cleanupTempDir(localPath);
    }
...
test('sync_folder honors includePatterns and excludePatterns', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      payload.details.skipped.map((entry: any) => ({ name: entry.name, reason: entry.reason })),
      [
...
test('sync_folder creates local directory for download when missing', async () => {
  ...
  try {
    const mock = createMockGraphClient({
      ...
      downloadFile: async () => ({ success: true, data: Buffer.from('remote') }),
    });
    __setGraphClientInstanceForTests(mock.client as any);
    ...
    assert.equal(readFileSync(path.join(localPath, 'report.txt'), 'utf8'), 'remote');
...
test('sync_folder deletes local and remote orphans when requested', async () => {
  ...
  try {
    ...
    assert.equal(mock.methodCalls('delete')[0].args[0], '/me/drive/items/remote-only-1');
    assert.deepEqual(
      payload.details.deleted.map((entry: any) => `${entry.location}:${entry.name}`).sort(),
      ['local:local-folder-only', 'local:local-only.txt', 'remote:remote-only.txt'],
    );
  } finally {
    cleanupTempDir(localPath);
  }
...
test('batch_file_operations covers successful operation branches sequentially', async () => {
  ...
  try {
    ...
    const mock = createMockGraphClient({
      uploadFile: async () => ({ success: true, data: { id: 'uploaded-1' } }),
      downloadFile: async () => ({ success: true, data: Buffer.from('downloaded-content') }),
    ...
    const response = (await handleBatchFileOperations({
      operations: [
        { operation: 'upload', source: uploadSource, destination: 'Docs/uploaded.txt' },
        { operation: 'download', source: 'Docs/server.txt', destination: downloadTarget, itemId: 'download-id' },
        { operation: 'move', itemId: 'move-id', destination: '/drive/root:/Archive' },
        { operation: 'copy', itemId: 'copy-id', destination: '/drive/root:/Copies', newName: 'copy-name.txt' },
    ...
    assert.equal(mock.methodCalls('uploadFile')[0].args[0], '/me/drive/root:/Docs/uploaded.txt:/content');
    assert.equal(mock.methodCalls('downloadFile')[0].args[0], '/me/drive/items/download-id/content');

    const patchCalls = mock.methodCalls('patch');
    assert.deepEqual(
      patchCalls.map((call) => [call.args[0], call.args[1]]),
      [
        ['/me/drive/items/move-id', { parentReference: { path: '/drive/root:/Archive' } }],
        ['/me/drive/items/rename-id', { name: 'renamed.txt' }],
      ],
    );

    assert.deepEqual(
      mock.methodCalls('post')[0].args,
      [
        '/me/drive/items/copy-id/copy',
        { parentReference: { path: '/drive/root:/Copies' }, name: 'copy-name.txt' },
      ],
    );
    assert.deepEqual(
      mock.methodCalls('delete').map((call) => call.args[0]),
      ['/me/drive/items/delete-id', '/me/drive/root:/Docs/delete-by-source.txt'],
    );
    assert.deepEqual(
  ...
  try {
    ...
    const response = (await handleBatchFileOperations({
      operations: [
        { operation: 'upload', source: uploadSource, destination: 'Docs/uploaded.txt' },
        { operation: 'explode' },
        { operation: 'upload', source: uploadSource, destination: 'Docs/uploaded-2.txt' },
      ],
      stopOnError: false,
    })) as ToolEnvelope;
    const payload = parsePayload<any>(response);
  ...
  try {
    ...
    const response = (await handleBatchFileOperations({
      operations: [
        ...
        { operation: 'upload', source: uploadSource, destination: 'Docs/should-not-run.txt' },
      ],
      stopOnError: true,
      parallel: false,
    })) as ToolEnvelope;
    ...
    assert.equal(response.isError, true);
    assert.match(response.content[0].text, /Source and destination required for download/);
    assert.equal(mock.methodCalls('uploadFile').length, 1);
...
test('batch_file_operations executes all operations in parallel and preserves per-item failures', async () => {
  ...
  try {
    ...
    const response = (await handleBatchFileOperations({
      operations: [
        { operation: 'upload', source: uploadSource, destination: 'Docs/uploaded.txt' },
        { operation: 'download', source: 'Docs/server.txt', destination: path.join(localPath, 'download.txt'), itemId: 'download-id' },
        { operation: 'delete', itemId: 'delete-id' },
      ],
      parallel: true,
      stopOnError: true,
...
test('batch_file_operations validates batch size and empty operations', async () => {
  ...
  const emptyResponse = (await handleBatchFileOperations({ operations: [] })) as ToolEnvelope;
  assert.equal(emptyResponse.isError, true);
  assert.match(emptyResponse.content[0].text, /At least one operation is required/);

  const oversizedResponse = (await handleBatchFileOperations({
    operations: Array.from({ length: 51 }, () => ({ operation: 'delete', itemId: 'x' })),
  })) as ToolEnvelope;
  assert.equal(oversizedResponse.isError, true);
  assert.match(oversizedResponse.content[0].text, /Maximum 50 operations allowed per batch/);
});

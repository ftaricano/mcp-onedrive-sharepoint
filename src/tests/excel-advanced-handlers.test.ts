...
test("excel_operations resolves itemPath, creates session, executes operation, and closes session", async () => {
  const mock = createMockGraphClient({
    get: async (
      ...
      if (
        endpoint ===
        "/sites/site-1/drive/items/excel-1/workbook/worksheets/SheetA/range(address='A1:B2')"
      ) {
        assert.deepEqual(params, {});
        assert.deepEqual(options, {
          headers: { "workbook-session-id": "session-1" },
        });
        return {
          success: true,
          data: {
  ...
  const scenarios = [
    {
      ...
      method: "patch",
      endpoint:
        "/me/drive/items/excel-1/workbook/worksheets/Sheet1/range(address='A1:B2')",
    ...
    },
    {
      ...
      method: "get",
      endpoint:
        "/me/drive/items/excel-1/workbook/worksheets/Sheet1/range(address='C1:C2')",
      resultCheck: (payload: any) => {
    ...
    },
    {
      ...
      resultCheck: (
        payload: any,
        mock: ReturnType<typeof createMockGraphClient>,
      ) => {
  ...
  for (const scenario of scenarios) {
    const mock = createMockGraphClient({
      ...
      post: async (
        ...
        if (
          endpoint ===
          "/me/drive/items/excel-1/workbook/worksheets/Sheet1/tables/add"
        ) {
        ...
        if (
          endpoint ===
          "/me/drive/items/excel-1/workbook/worksheets/Sheet1/charts/add"
        ) {
          assert.deepEqual(data, {
            type: "LineMarkers",
            sourceData: "Sheet1!A1:B4",
            seriesBy: "auto",
          });
          return {
            success: true,
    ...
    assert.ok(
      calls.some((call) => call.args[0] === scenario.endpoint),
      scenario.name,
    );
  }
});
...
test("excel_operations validates required fields per operation and rejects invalid operations", async () => {
  ...
  for (const validationCase of validationCases) {
    ...
    assert.match(
      ...
      new RegExp(validationCase.error.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
    );
  }
});
  ...
  const scenarios = [
    {
      ...
      get: async (endpoint: string) => {
        assert.equal(
          endpoint,
          "/me/drive/items/excel-1/workbook/worksheets/Sheet1/range(address='A1:B3')",
        );
        return {
          success: true,
          data: {
    ...
    },
    {
      ...
      get: async (endpoint: string) => {
        assert.equal(
          endpoint,
          "/me/drive/items/excel-1/workbook/worksheets/Pivot/pivotTables",
        );
        return {
          success: true,
          data: {
      ...
      post: async (endpoint: string) => {
        assert.equal(
          endpoint,
          "/me/drive/items/excel-1/workbook/worksheets/Pivot/pivotTables/pivot-1/refresh",
        );
        return { success: true };
      },
    ...
    },
    {
      ...
      get: async (endpoint: string) => {
        assert.equal(
          endpoint,
          "/me/drive/items/excel-1/workbook/worksheets/Sheet1/usedRange/dataValidation",
        );
        return {
          success: true,
          data: {
    ...
    },
    {
      ...
      get: async (endpoint: string) => {
        ...
        assert.equal(
          endpoint,
          "/sites/site-1/drive/items/excel-1/workbook/names",
        );
        return {
          success: true,
          data: {
    ...
    },
    {
      ...
      get: async (endpoint: string) => {
        assert.equal(
          endpoint,
          "/me/drive/items/excel-1/workbook/worksheets/Sheet1/usedRange",
        );
        return {
          success: true,
          data: {
...
test("excel_analysis rejects missing item reference and invalid analysis type", async () => {
  ...
  const missingItemResponse = (await handleExcelAnalysis({
    analysisType: "used_range",
  })) as ToolEnvelope;
  assert.equal(missingItemResponse.isError, true);
  assert.match(
    missingItemResponse.content[0].text,
    /Either itemId or itemPath is required/,
  );

  mock = createMockGraphClient();
  __setGraphClientInstanceForTests(mock.client as any);

  const invalidTypeResponse = (await handleExcelAnalysis({
    itemId: "excel-1",
    analysisType: "bogus",
  })) as ToolEnvelope;
  assert.equal(invalidTypeResponse.isError, true);
  assert.match(
    invalidTypeResponse.content[0].text,
    /Invalid analysis type: bogus/,
  );
});

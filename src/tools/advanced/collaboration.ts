...
export const advancedShare: Tool = {
  name: 'advanced_share',
  description: 'Create advanced sharing links with custom permissions and email notifications',
  inputSchema: {
    type: 'object',
    properties: {
      itemId: {
...
export async function handleAdvancedShare(args: any) {
  try {
    ...
    if (itemId) {
      endpoint = siteId 
        ? `/sites/${siteId}/drive/items/${itemId}/invite`
        : `/me/drive/items/${itemId}/invite`;
    } else if (itemPath) {
      endpoint = siteId
        ? `/sites/${siteId}/drive/root:/${itemPath}:/invite`
        : `/me/drive/root:/${itemPath}:/invite`;
    } else {
    ...
    switch (action) {
      case 'list': {
        ...
        const response = await client.get<any>(endpoint, {
          '$expand': 'grantedTo,grantedToIdentities'
        });
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      const userPermissions = allPermissions.filter((perm: any) => {
        ...
        if (perm.grantedTo?.user?.email?.toLowerCase() === userEmail.toLowerCase()) {
          return true;
        }
        
        // Check granted to identities (for shared links)
        if (perm.grantedToIdentities) {
          return perm.grantedToIdentities.some((identity: any) => 
      ...
      if (includeInherited) {
        inheritedPermissions = allPermissions.filter((perm: any) => 
          perm.inheritedFrom && !userPermissions.includes(perm)
        );
      }

      const hasAccess = userPermissions.length > 0 || inheritedPermissions.length > 0;
